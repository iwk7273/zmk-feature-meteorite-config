...
ZMK Studioでは「meteorite custom config」として表示される。パラメータは一覧から選択し、`Select config op`（値0）のままだと何もしない。
...
旧`C_SCRL1_UP`（10）と`C_SCRL2_UP`（11）は互換性のため番号だけを予約し、非表示のno-opとして受理します。
...
- `scroll_layer_1/2`は旧firmwareとの互換性のためだけに残す凍結フィールドです。現行のルーティングには使わず、editorでも表示しませんが、NVS/RPC上の値は保持します。
...
### 5) Meteorite入力処理
Meteorite40のトラックボール処理は、このモジュール内の入力デバイスとinput processorをDTSで組み合わせる。
...
| `zmk,input-meteorite-sensor-rotation` | raw X/Yをフレーム単位で回転し、完全な2軸イベントとして再発行する。 |
| `zmk,input-processor-meteorite-scroll-transform` | raw X/Y入力から物理速度を求め、スクロール応答、蓄積、軸選択、wheel変換を一括処理する。 |

`zmk,input-meteorite-sensor-rotation`は`device`で指定したraw入力を`sync`まで蓄積し、
同一フレームのX/Yを回転してX（`sync=false`）、Y（`sync=true`）の順に再発行する。
X/Y別の固定小数点端数を持ち越すため、低速の1 count入力も累積では失われない。
角度変更時には旧角度の端数を破棄する。
...
SCROLL chainでは`zmk,input-processor-meteorite-scroll-transform`を単独で使う。raw countsにgainを適用した後でQ16 accumulatorからwheel stepを取り出すため、ゆっくりした1 count入力も失わずに蓄積できる。
