...
struct meteorite_xy_clipper_data {
    int32_t x;
    int32_t y;
    bool threshold_warned; /* gate the invalid-threshold warning to one emission */
...
static int meteorite_xy_clipper_handle_event(const struct device *dev, struct input_event *event,
    ...
    if (y_triggered && (!x_triggered || (abs_y * 2) >= abs_x)) {
        ...
        data->y %= threshold;
        data->x = 0;
        LOG_DBG("meteorite_xy_clipper choose Y val=%d rem_y=%d reset_x=0 sync=%d", event->value,
                data->y, event->sync);
        return ZMK_INPUT_PROC_CONTINUE;
    } else if (x_triggered) {
        ...
        data->x %= threshold;
        data->y = 0;
        LOG_DBG("meteorite_xy_clipper choose X val=%d rem_x=%d reset_y=0 sync=%d", event->value,
                data->x, event->sync);
        return ZMK_INPUT_PROC_CONTINUE;
    }
